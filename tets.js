    ...
    function initDBInfo() {
        DBInfo = {
            ...
            openAttackDetail() {
                ...
                incTable.each((index, element) => {
                    if (index === 0 || index === incTable.length - 1) return;
                    ...
                    setTimeout(() => {
                        ...
                        element.insertAdjacentHTML('beforeend', `<td id="dbinfo-${index}"></td>`);
                        DBInfo.showDatabaseDetails(coords[0], coords[1], (data) => this.showAttackDetail(data, coords[0], coords[1], arrival, index), arrival);
                    }, 200 * index);
                });
            },
            showAttackDetail(data, x, y, arrival, index) {
                ...
                possibleAttacks.forEach(element => {
                    ...
                    switch (element.type) {
                        case 1: 
                            incType = "<span style='color: green;'>Fake</span>";
                            break;
                        case 2: 
                            incType = "<span style='color: red;'>mögliche Off</span>";
                            break;
                        case 3: 
                            incType = "Off";
                            break;
                    ...
                    message += "Angriff von: " + DBInfo.escapeHTML(element.attacker_koords) + " Typ: " + incType + " Grund: " + DBInfo.escapeHTML(element.reason) + "<br>";
                });
            
                if (message) {
                    $(`#dbinfo-${index}`).html(message);
                } else {
                    $(`#dbinfo-${index}`).text("Keine Daten verfügbar.");
                }
            },
