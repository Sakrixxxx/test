        ...
        DBInfo = {
            ...
            openAttackDetail() {
                ...
                for (let index = 1; index < incTable.length - 1; index++) {
                    ...
                    element.insertAdjacentHTML('beforeend', `<td id="db-info-${index}">Lade...</td>`);
            
                    // Automatischer Aufruf der showDatabaseDetails-Funktion mit zusätzlichem Index-Parameter
                    DBInfo.showDatabaseDetails(coords[0], coords[1], (data) => DBInfo.showAttackDetail(data, coords[0], coords[1], arrival, index), arrival);
                }
            },
            showAttackDetail(data, x, y, arrival, index) {
                ...
                var dbInfoCell = document.getElementById(`db-info-${index}`);
                if (message) {
                    dbInfoCell.innerText = message;
                } else {
                    dbInfoCell.innerText = "Keine Daten";
                }
            },
